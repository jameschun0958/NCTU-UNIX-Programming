...
int sigaction(int how, struct sigaction const *nact, struct sigaction *oact)
{
    struct sigaction kact, koact;

    if (nact)
    {
        kact.sa_handler = nact->sa_handler;
        sigprocmask(0, &nact->sa_mask, &kact.sa_mask);
        kact.sa_flags = nact->sa_flags | SA_RESTORER;
        kact.sa_restorer = sys_sigreturn;
    }

    // nact->sa_flags |= SA_RESTORER;
	// nact->sa_restorer = sigreturn;
    
    long ret = sys_rt_sigaction(how, &kact, &oact, sizeof(sigset_t));
    WRAPPER_RETval(unsigned int);
    
    /*
    nact->sa_flags |= SA_RESTORER;
    nact->sa_restorer = sys_sigreturn;
...
// long sigreturn(unsigned long __unused)
// {
//     long ret = sys_rt_sigreturn(__unused);
//     WRAPPER_RETval(unsigned int);
// }